# 第 29 轮 · 实施清单

## 授权与分支

- [x] 用户确认用户名＋密码＋邀请码交互稿，并要求在新分支开发。
- [x] App 分支：`feat/account-auth-service`，保留此前未提交的公共规则整理。
- [x] 独立服务本地仓库：`/zhangaimin/Agentic-KeepKeep/Moveliq-Service`，同名功能分支；服务源码不进入 App 仓库。
- [x] GitHub 私有仓库 [MinJung-Go/Moveliq-Service](https://github.com/MinJung-Go/Moveliq-Service) 已创建；`feat/account-auth-service` 首推提交 `cb2b880`。

## 服务端

- [x] Node.js HTTP 服务与 PostgreSQL 持久化；Vercel 和独立服务器共用入口。
- [x] 数据库迁移：用户、会话、邀请码、跨实例限流。
- [x] 用户名标准化与唯一性、密码安全哈希、统一登录失败文案。
- [x] 注册事务：锁定并消费原码、创建用户、发放 3 个子码；失败回滚。
- [x] 重试不追加码，同码并发注册仅一人成功；子邀请逐层延续。
- [x] 登录、会话恢复、退出撤销、邀请码列表；过期和禁用账号拒绝访问。
- [x] 管理员 CLI：种子邀请码、禁用账号、密码重置（撤销旧会话），不开放公网管理接口。
- [x] 鉴权模型代理：固定上游、服务端模型密钥、请求限额、超时与客户端取消。
- [x] 普通／流式聊天、图片和工具调用；网页搜索／阅读通过同一服务鉴权。
- [x] 配置与部署文档、环境变量示例、Docker、自有 PostgreSQL／Vercel 接入说明。

## App 认证与 UI

- [x] 服务地址由构建配置提供；默认接入指定 8080 地址，临时 HTTP 精确白名单＋ATS 单 IP 例外；不硬编码供应商 Key。
- [x] 认证客户端、Keychain 会话、启动恢复、网络失败重试和 401 失效。
- [x] 按已确认交互稿实现用户名密码登录、邀请码注册、加载／错误与密码显示。
- [x] 我的邀请码：三份码、复制、可用／已使用状态。
- [x] 账号设置与退出确认；忘记密码提供管理员协助入口说明。
- [x] 登录成功后进入既有引导／主界面，移除 API Key 引导页。

## 账号数据与模型迁移

- [x] 明确旧记录归属与账号隔离策略。
- [x] 旧记录关联须用户确认；更换账号隔离数据，退出锁定 Widget，不删旧记录。
- [x] 关闭本地模型入口与请求路由，迁移旧偏好。
- [x] 停止后台模型下载恢复／新建与运行时加载；保留代码和已下载文件。
- [x] 通过统一代理调用所有 Agent 的云端请求与搜索／阅读。
- [x] 更新隐私、README、AGENTS 与接入说明的架构描述。

## 验证与交付

- [x] 服务真实 PostgreSQL 事务测试：2026-09-22 在服务器独立 PostgreSQL 17 测试库运行，9/9 通过，包含并发抢码、回滚、唯一性、逐层发码与重试。
- [x] 服务 API 测试：认证、撤销、限流、上游错误／取消／流式与密钥隔离（mock 上游）。
- [x] App 认证解码、输入策略、个人偏好隔离：可移植 XCTest 6 项通过（新增 HTTP 地址白名单）。
- [x] 编写 App 认证 HTTP 3 项测试，并调整现有模型路由／预算测试；已随 iOS CI 通过。
- [ ] SwiftData 旧记录关联、分库、Keychain 与 Widget 的 iOS 集成验收。
- [x] Swift 6 语法检查 164 个文件；构建 YAML、Info.plist 与 `git diff --check` 通过。
- [x] iOS CI 构建／全量单测：`e0cc02c`，429 项通过、0 失败，[运行记录](https://github.com/MinJung-Go/Agentic-KeepKeep/actions/runs/35724014924)。
- [ ] 真机登录、Keychain、Widget、后台下载停用与流式回归（未执行不宣称通过）。

## 设计依据

[交互稿](design.html) 已通过 21 项浏览器流程检查。用户名 4–24 位、密码 12–128 位作为本轮实现规则；微信与短信不接入。设计验证不替代后端或 iOS 验证。

## 当前验证记录（2026-09-22）

- App：`feat/account-auth-service` 已推送；代码提交 `e0cc02c` 已通过 Apple 编译和 **429/429** 单测。后续仅回填文档记录。
- 独立服务：同名分支已推送 `cb2b880`；PGlite＋mock HTTP 测试 **9/9** 通过。
- 服务本地事务测试覆盖重试、逐层发码、用户名冲突与发码中途失败回滚；PGlite 单连接串行测试，该本地结果不证明真实并发；后续已在服务器独立 PostgreSQL 17 测试库通过多连接测试，服务 CI 状态仍未确认。
- Swift 可移植测试 **6/6**；新认证 HTTP 测试与更新的本地模型停用／预算测试亦随 Apple CI 通过。
- 本地 Swift 语法检查之外，已由上述 macOS CI 完成 Apple SDK 编译、链接及模拟器单测；不代表真机验收。
- 已在自有服务器部署独立服务与真实 PostgreSQL；GLM 环境变量已配置，真实文本／鉴权流式调用通过；公网 HTTPS 未配置，未发布 IPA。
- 管理员 CLI、环境配置、Vercel／Docker 说明在独立仓库 README；App 接入见 [integration.md](integration.md)。

## 剩余验收

1. 配置 HTTPS 域名后覆盖 `MOVELIQ_SERVICE_URL`，移除临时 HTTP 例外；当前默认 IP:8080 可用于用户指定的临时接入。
2. 真机验收登录、邀请、旧记录归属、账号切换、Widget、后台下载停用、AI 图片／工具与流式。
3. 如需网页检索，开启服务端搜索配置并联调；本轮不自动启用。
4. 合并／发布 IPA 尚未执行。

自有服务器部署与验收详见 [integration.md](integration.md)。

## App 接入与推送追加

- [x] 用户授权适配 App 后提交／推送；默认服务 `http://47.100.234.212:8080`，CI 同默认。
- [x] HTTP 仅允许该精确地址，其他 HTTP 被拒绝；ATS 仅对该 IP 配置例外。
- [x] GLM Key 配置于服务器受限权限环境文件；`glm-5.3-flash` 真实文本请求 200／OK。
- [x] `e0cc02c` iOS CI 构建成功，429 项单测全部通过。

- 首轮 Apple 编译发现已移除 API Key 控件仍残留 `isKeyVisible` 回调；已删除该回调并重新运行 CI。真实服务注册→3 个邀请码→GLM SSE→退出撤销已联调通过。

## 短邀请码追加

- [x] 用户确认 8 位分组短码方案，同步需求和设计示例。
- [x] 新码生成、格式归一化、旧码兼容、冲突重试与验证：Service `b5d06bd`，18 项服务测试／14 项浏览器检查通过。
- [x] App 输入归一化和提示更新；Swift 可移植认证测试 7/7 通过。版本 0.5.5（26）。
- [x] 更新 App 输入提示和提交规则；主仓库 `c2fa7ab` 已 push，tag `v0.5.5` 已发布；iOS CI 35821859967 构建及 430 项单测通过。
- [x] 服务短码版本 `b5d06bd` 已部署，后续管理员更换 Key 版本 `b9af30b` 同样已部署；服务不运行 Actions。
- [x] v0.5.5 IPA 归档、下载和产物核验：CI 35821859967 全部成功，产物 10734076962；版本 0.5.5（26）、iOS 17.0、包大小 8,736,263 字节，Widget／MLX 资源和许可证检查通过，未打包模型权重。未做真机安装验证，IPA 需要侧载重签名。
- [下载 IPA](https://github.com/MinJung-Go/Agentic-KeepKeep/actions/runs/35821859967/artifacts/10734076962)。
- 管理员密钥更换功能由 Service 独立实现并部署，不改变本次 App 包。
